# Collaborative Layout Designer — Web

A web port of [Collaborative Layout Designer](https://github.com/aronwk-aaron/collaborative-layout-designer)
(BlueBrick layout editor) with Google / GitHub / OIDC sign-in, real-time
multi-user editing, organisations, layout sharing, custom parts, and saved
modules. Self-hosted via `docker compose up`. Save-format compatible with the
desktop CLD's `.bbm` + sidecar pair.
...
> - **Bugs may be subtle.** The test suite (195 tests) covers a lot, but
>   AI-generated code has a knack for hiding edge cases behind
>   confident-looking comments.
...
- Browser-based brick layout editor (same model as desktop CLD)
...
See [PLAN.md](./PLAN.md) for the full design / decisions / phase breakdown.
...
- **Docker + docker compose** (only for the containerised deploy)
...
```sh
# 1. Clone with submodules — the BlueBrickParts library is a submodule.
git clone --recurse-submodules https://github.com/aronwk-aaron/collaborative-layout-designer-web.git
cd collaborative-layout-designer-web
...
|----------------|-------------|--------------------------------------|
| `cld-data`     | `/data`     | SQLite database file                 |
| `cld-backups`  | `/backups`  | Daily gzipped DB snapshots           |
| `parts-library`| `/parts`    | BlueBrickParts library (read-only)   |
...
| `DB_PATH`                 | `./data/cld.sqlite`    | SQLite file path                                   |
...
```sh
# Run the full test suite (195 tests across all workspaces)
pnpm test
...
packages/
  ...
  parts-catalog/    BlueBrickParts XML scanner
  ydoc/             Yjs ↔ model projection
parts-library/      Submodule: BlueBrickParts (XML + sprites)
...
- **`ci.yml`** — runs on every PR: lint, typecheck, build, test,
  dependency review, OSV scan, secret scan, CodeQL, Trivy.
...
[AGPL-3.0-or-later](./LICENSE). Same as the desktop CLD it's ported from.

The bundled `parts-library/` is a submodule of upstream
[BlueBrickParts](https://github.com/Lswbanban/BlueBrickParts) and ships
